...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{21em}
		The use of curvilinear elements can be very effective for some geometries. In the classical finite element approach, rounded lines have to be approximated by a sequence of small straight segments. In the figure left we can see example of such mesh. Approximation of the circular parts would result in loss of accuracy and also in creation of large amount of elements in the initial mesh. The use of curvilinear elements prevents this and hence leads to faster and more accurate calculations.
	\end{minipage}
\end{center}
}
...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{18em}
		In the figure above, we can see several configurations of an oven used for induction heating. The schema (and also the calculations) are axisymetric.
Several parameters can be changed -- the shape of the oven (height and diameter) can vary, but its volume remains constant. Also the position and size of inductors (two bars on the right hand side of the figure) can be changed. In the figure left we can  see magnetic field generated by the inductor and temperature field generated by Joule losses in the material. It is an example of weak coupling, where values of one field are used to calculate another field.
	\end{minipage}
\end{center}

\begin{center}
	\begin{minipage}{22em}
		In this problem, there are two different indicators of the quality of the design. The total heat generated in the  oven should be as big as possible and should be equally distributed. In the figure, each point represents one calculation and its resulting heat $Q$ (should be maximized) and unevenness of distribution of heat $U$ (should be minimized). Blue points represent calculations with randomly chosen parameters. Red points are products of optimization. Using different weights for $Q$ and $U$, optimal points form so-called Pareto front. There are more sophisticated algorithms to find the Pareto front, which will be implemented in the future.
	\end{minipage}
	\begin{minipage}{28em}
		\centering
...
\begin{center}
	...
	\end{minipage}
	\begin{minipage}{16.5em}
		Electric field potential is calculated first. $hp$-adaptivity has been used, since it is very efficient for electrostatic problems. In the figure below we can see a fine mesh, that has been created by automatic adaptivity algorithm. It can be seen, that the mesh is very fine close to the singularities of the field and that various polynomial degrees are used in order to minimize the solution error.
	\end{minipage}
\end{center}
